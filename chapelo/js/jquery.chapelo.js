...
(function ($) {
    ...
    function Chapelo(field, alphabet, suffixes, diphthongs) {
        ...
        this.replaceLast = function() {
            // Just replace the last character typed
            if (this.switchedOff || !this.mustReplace('field')) { return }
            var caret = this.caretPosition;

            if (this.mustReplace('suffix')) {
                var text = this.field.value;
                var chars = text.slice(caret-2, caret);
                var regex = this.getRegex(this.alphabet, this.suffixes);
                if (chars.match(regex)) {
                    var char = chars.replace(regex, this.encode);
                    this.field.value = text.slice(0, caret-2) + char + text.slice(caret);
                    this.setCaret(chars, regex, this.caretPosition);
                }
            }
            ...
            if (this.mustReplace('diphthong')) {
                ...
                var chars = text.slice(caret-2, caret);
                var regexDiphthong = this.getRegex(this.diphthongs);
                if (chars.match(regexDiphthong)) {
                    var char = chars.replace(regexDiphthong, this.encodeDiphthong);
                    this.field.value = text.slice(0, caret-2) + char + text.slice(caret);
                    this.setCaret(chars, regex, this.caretPosition, true);
                }
            }
        ...
        this.cancel = function(e) {
            ...
            var caret = this.caretPosition;
            var text = this.field.value;
            var char = text.slice(caret-1, caret);
            var unHattedChar = removeHat(char);
            console.log(unHattedChar);
            if (unHattedChar != ''){
                this.field.value = text.slice(0, caret-1) + unHattedChar + text.slice(caret);
                e.preventDefault();
            }
    ...
    $.fn.chapelo = function(initial_options) {
        ...
        return this.filter(options.selectors)
            ...
            $(this).keydown(function(e) {
                var backspace = 8;
                var x = 88;
                if (e.keyCode === backspace | e.keyCode === x) {
                    console.log('backspace');
                    new Chapelo(this, options.alphabet, options.suffixes, options.diphthongs).cancel(e);
                }
                else {
                    $(this).keyup(function(e) {
                        new Chapelo(this, options.alphabet, options.suffixes, options.diphthongs).replaceLast();
                    });
                }
            });
        });
